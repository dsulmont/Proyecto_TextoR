...
Las tablas de frecuencias, simples o cruzadas, son una de las principales herramientas para el análisis estadístico descriptivo. En este capítulo veremos cómo producirlas utilizando el R
...
\section{Diseño muestral y ponderación de datos}
...
En el siguiente comando estamos incluyendo el factor de expansión de la ENAHO. Como puede apreciarse, al tomar en cuenta el diseño muestral y el peso de cada hogar seleccionado respecto de la población que busca representar, el porcentajes de hogares que tienen acceso a servicios higiénico conectado a la red pública en la vivienda pasa a 64.6\%.

<<>>=
desague2 <- xtabs(FACTOR07 ~ P111A, data = enaho15_1)
desague2
prop.table(desague2)*100
@


\section{Tabulaciones cruzadas o tablas de contingencia}

Una tabla de frecuencias cruzadas o tabla de contingencia nos sirve para examinar cómo están distribuidas los casos de un conjunto de datos entre los valores de dos variables categóricas. Específicamente, las tablas de contingencia nos ayudan a observar cómo la distribución de las categorías de una variable de análisis o variable dependiente cambia cuando introducimos otra variable de control o variable dependiente.

Para construir una tabla de contingencia o tabulación cruzada es necesario tomar en cuenta los siguientes pasos:

\begin{itemize}
  \item Especificar cuál es nuestra variable de análisis o variable dependiente, aquella cuya distribución de valores o categorías nos interesa describir.
  \item Especificar cuál es nuestra variable de control o variable independiente, aquella que nos sirve para comparar la distribución de nuestra variable de análisis entre diferentes grupos definidos por nuestra variable de control.
  \item Especificar cómo deben colocarse las variables en las dimensiones de la tabla (las filas o las columnas) y cómo deben calcularse los porcentajes en una tabla.
\end{itemize}

Para analizar o leer una tabla de contingencia, las frecuencias deben presentarse en porcentajes. En una tabulación cruzada los porcentajes pueden calcularse tomando como base:

\begin{itemize}
  \item El total de casos de la tabla.
  \item El total de casos de cada categoría de las filas.
  \item El total de casos de cada categoría de las columnas.
\end{itemize}

Sin embargo, la regla para el cálculo de porcentajes en una tabla de contingencia es la siguiente: los porcentajes deben calcularse sobre la base de cada categoría de la variable de control o variable independiente. 

A continuación mostraremos cómo se elabora una tabla de contingencia utilizando diferentes comandos del R. Para nuestro ejemplo consideraremos las siguientes variables tomadas de la base de datos sobre Familia y Roles de Género:

\begin{itemize}
  \item Variable de análisis o variable dependiente: Opinión sobre quién debe cuidar a los miembros de la familia que están enfermos (pregunta P35C)
  \item Variable de control o variable independiente: Sexo del entrevistado
\end{itemize}


\subsection{Tabulación cruzada con el comando \texttt{table}}

El primer paso en la elaboración de una tabla de contingencia es construir una tabla cruzada de frecuencias absolutas con el comando table. Como convención general pondremos las categorías de la variable de análisis en las filas y las de la variable de control en las columnas:
...
El siguiente paso es convertir la tabla de frencuencias absolutas en una de frecuencias relativas o proporciones utilizando el comando \texttt{prop.table}. Este comando nos proporciona tres opciones para el cálculo de proporciones:

a) Proporciones calculadas para el total de casos de la tabla

<<>>=
prop.table(tab_cruz1)
@

b) Proporciones calculadas para cada categoría de las filas

<<>>=
prop.table(tab_cruz1, 1)
@

c) Proporciones calculadas para cada categoría de las columnas

<<>>=
prop.table(tab_cruz1, 2)
@


En nuestro ejemplo, siguiendo nuestra regla, la forma correcta de calcular las proporciones sería la tercera: para cada categoría de las columnas. Para convertir las proporciones en porcentajes se debe multiplicar toda la tabla por 100:

<<>>=
prop.table(tab_cruz1, 2)*100
@

Tabmbién es posible llegar al mismo resultado anidando los comandos:

<<results=hide>>=
prop.table(table(genero12$P35C, genero12$SEXO),2)*100
@

Para leer la tabla de contingencia es necesario comparar los porcentajes en el sentido contrario al que fueron calculados. En nuestro ejemplo observamos que el porcentaje de personas que opinan que son las mujeres las que deben cuidar a los miembros enfermos de la familia es mayor entre las entrevistadas de sexo femenino que entre los varones; lo contrario ocurre con la respuesta ``ambos igual''.


\subsection{Tabulación cruzada con el comando \texttt{xtabs}}

Con el comando \texttt{xtabs} podemos crear la misma tabla de frecuencias cruzadas que vimos previamente de la siguiente manera:

<<>>=
tab_cruz1.b <- xtabs(~ P35C + SEXO, data = genero12)
tab_cruz1.b
prop.table(tab_cruz1.b, 2)*100
@

También es posible llegar al mismo resultado con:

<<results=hide>>=
prop.table(xtabs(~ P35C + SEXO, data = genero12),2)*100
@

El comando \texttt{xtabs} nos pemite construir tablas cruzadas considerando un factor de expansión. En el siguiente ejemplo se utiliza los datos de la ENAHO 2015 para mostrar como cada estratos socioeconómico está distribuido entre los dominios geográficos. Los porcentajes tienen como base cada estrato (columna) y los datos están expandidos. Como se puede ver, los hogares de los estratos socioeconómicos más altos (A y B) están básicamente concentrados en la capital del país, Lima Metropolitana.

<<>>=
prop.table(xtabs(FACTOR07 ~  DOMINIO + ESTRSOCIAL, data = enaho15_1), 2)*100
@

Si queremos conocer cuál es la composición por niveles socioeconómicos en cada dominio grográfico podemos calcular los porcenjares para fila. Aquí se puede apreciar que los estratos A y B representan el 13.8\% de los hogares de Lima Metropolitana.

<<>>=
prop.table(xtabs(FACTOR07 ~  DOMINIO + ESTRSOCIAL, data = enaho15_1), 1)*100
@
